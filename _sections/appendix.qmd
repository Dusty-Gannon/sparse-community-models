...
$$
\hat\mu_\xi^{(t+1)} = \frac{\mathbb{E}^{(t)}({\bf r})^\top \mathbb{E}^{(t)}({\bf W}^{-1}) {\bf Z}\ \mathbb{E}^{(t)}(\boldsymbol \psi)}{\mathbb{E}^{(t)}({\boldsymbol \psi}^\top {\bf Z}^\top {\bf W}^{-1} {\bf Z} \boldsymbol \psi) + \mathbb{E}^{(t)}(c^{-2})}
$$ {#eq-update-xi-mean}
and
$$
\hat{v}_{\xi}^{(t+1)} = \left(\mathbb{E}^{(t)}({\boldsymbol \psi}^\top {\bf Z}^\top {\bf W}^{-1} {\bf Z} \boldsymbol \psi) + \mathbb{E}^{(t)}(c^{-2}) \right)^{-1}
$$ {#eq-update-xi-var}
...
Finally, we can update the error variances, which, for $\sigma_1^2,\sigma_2^2,...,\sigma_S^2$ we give improper Jeffrey's priors of $1/\sigma_i^2$ for $i = 1,2,...,S$. The conditional posterior is therefore inverse gamma, with constant shape parameter $n/2$ and updates to the rate parameter of

$$
b_{\sigma_i} = \frac{1}{2}\left({\bf y}_{[i]}^\top{\bf y}_{[i]} - {\bf y}_{[i]}^\top\tilde{\bf X}_{[i]} \mathbb{E}(\tilde {\boldsymbol \beta}) - \mathbb{E}(\tilde{\boldsymbol \beta})^\top \tilde{\bf X}_{[i]}^\top {\bf y}_{[i]} + \mathbb{E}(\tilde{\boldsymbol \beta})^\top \tilde{\bf X}_{[i]}^\top \tilde{\bf X}\ \mathbb{E}(\tilde{\boldsymbol \beta})\right)
$$
