...
### Conditional posteriors and updates for latend indicators {.appendix}
...
$$
where $\bf r$ is the vector of conditional residuals, $r_i = (y_i - \tilde{\bf X}_i^\top \tilde{\boldsymbol \beta})$. We can therefore derive the variational updates by
$$
q(\xi|\boldsymbol \theta_{-\xi},{\bf y}) = \exp\left\{\mathbb{E}\left[ \log p(\xi\ |\ \boldsymbol \theta_{-\xi},{\bf y}) \right]\right\}
$$
<!-- Start fixing here!!! --> 

which yields the updates
$$
\hat\mu_\xi^{(t+1)} = \frac{\mathbb{E}^{(t)}({\bf r})^\top Z_m^\top \mathbb{E}^{(t)}(\psi_m)}{\mathbb{E}^{(t)}(\psi_m^2){\bf Z}_m^\top{\bf Z}_m + \mathbb{E}^{(t)}(c^{-2})}
$$ {#eq-update-xi-mean}
and
$$
\hat{v}_{\xi}^{(t+1)} = \left(\mathbb{E}^{(t)}(\psi_m^2){\bf Z}_m^\top{\bf Z}_m + \mathbb{E}^{(t)}(c^{-2}) \right)^{-1}
$$ {#eq-update-xi-var}
